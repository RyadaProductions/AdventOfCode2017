...
namespace Day4
{
    class Program
    {
        ...
        {
            var passwords = input.Split(new[] { Environment.NewLine }, StringSplitOptions.None);

            for (var i = 0; i < passwords.Length; i++)
            {
                // part 1
                var words = passwords[i].Split(' ');
                if (words.Length == words.Distinct().Count()) _solution++;
                ...
                {
                    var chars = words[j].ToLower().ToCharArray();
                    Array.Sort(chars);
                    test.Add(new string(chars));
                }

                if (test.Count == test.Distinct().Count()) _solution2++;
            }
